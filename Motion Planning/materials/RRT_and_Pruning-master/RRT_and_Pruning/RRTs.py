...
class RRTPlanner():
    ...
    def RRTSearch(self):
        ...
        for i in range(self.N):
            if(random.random()>=1.95): #  [0.0, 1.0), Change to a value under 1 to bias search towards goal, right now this line doesn't run
    ...
    ******************************************************************************************************************************************
    ...
    def steer(self, from_point, to_point):
        ...
        if fromPoint_buffered.distance(toPoint_buffered) < self.steer_distance:
            ...
            to_x, to_y = to_point
            distance = self.euclidian_dist(from_point, to_point)
            unit_direction_vec = ((to_x - from_x) / distance, (to_y - from_y) / distance)
            new_point = (from_x + self.steer_distance * unit_direction_vec[0], \
                       from_y + self.steer_distance * unit_direction_vec[1])
            return new_point
