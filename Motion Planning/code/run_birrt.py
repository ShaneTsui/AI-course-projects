...
from RobotPlanner_AStar import AStarPlanner
from RobotPlanner_BiRRT import BiRRTRobotPlanner
from RobotPlanner_RRT import RRTRobotPlanner
...
def runtest(mapfile, start, goal, verbose = True):
    ...
    RP = BiRRTRobotPlanner(boundary, blocks)
    ...
    # Stop at start point until the planning is finished
    RP.init_params(start, goal, ax, fig, plt)
    t0 = tic()
    while True:
        if RP.pre_plan():
           break
    plan_time = np.ceil(tic() - t0)
    ...
    plt.savefig("./images/RRT-"+str(mapfile.split('/')[-1].split('.')[0])+"-"+ str(total_distance)+ "-"+ str(numofmoves)+ "-"+str(plan_time)+ ".png")
    plt.close()
